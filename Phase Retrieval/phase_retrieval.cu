...
__global__ void FFTShift2D( cufftComplex* input, cufftComplex* output, int numElements) {
	...
	if (i < numElements) {
		if (y >= 480) {
			...
			if (x >= half) {
				output[y + x * blockDim.y].x = input[480 - y + 481 * (1280 - x + 640)].x;
				output[y + x * blockDim.y].y = input[480 - y + 481 * (1280 - x + 640)].y;
			}
			else {
				output[y + x * blockDim.y].x = input[480 - y + 481 * (640 - x)].x;
				output[y + x * blockDim.y].y = input[480 - y + 481 * (640 - x)].y;
			}
		}
	}
}
...
float* phaseRetrieval(image* calibImage, image* testImage) {
	...
	if (cudaSuccess != cudaMemcpy(phaseImage, dev_phaseImage, testImage->imagePixels * sizeof(float), cudaMemcpyDeviceToHost))
		cout << "cuda memory cpy error!" << endl;
	realWrite("phase image", phaseImage, 960, "..\ouput_text\phase_image1.csv");
	/*
