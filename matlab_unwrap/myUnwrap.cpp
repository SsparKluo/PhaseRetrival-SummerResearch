    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("myUnwrap"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return myUnwrapInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
LIB_myUnwrap_CPP_API 
void MW_CALL_CONV myUnwrap(int nargout, mwArray& phaseImage, const mwArray& input)
{
    mclcppMlfFeval(_mcr_inst, "myUnwrap", nargout, 1, 1, &phaseImage, &input);
}
